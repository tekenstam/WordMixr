...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:5173"],  # Add Vite dev server port
...
async def startup_event():
    ...
    logger.info(f"Dictionary configuration: {Config.get_dictionary_info()['description']}")
    ...
    try:
        ...
        logger.info(f"Dictionary: {DICTIONARY_INFO['filepath']} ({DICTIONARY_INFO['size']} words)")
...
async def solve_puzzle(
    letters: str = Query(..., description="Scrambled letters to solve", min_length=1, max_length=20),
    min_word_length: int = Query(3, description="Minimum word length to include in results", ge=1, le=10)
):
    """
    ...
    try:
        ...
        dict_type = DICTIONARY_INFO.get("type", "scowl_large") if DICTIONARY_INFO else "scowl_large"
        valid_words = find_valid_words(cleaned_letters, DICTIONARY, min_word_length, dict_type)
        
        logger.info(f"Found {len(valid_words)} words for letters: {cleaned_letters} (min length: {min_word_length})")
...
async def find_anagrams(
    letters: str = Query(..., description="Letters to find anagrams for", min_length=1, max_length=20),
    min_word_length: int = Query(3, description="Minimum word length to include in results", ge=1, le=10)
):
    """
    ...
    try:
        ...
        logger.info(f"Found {len(anagrams)} anagrams for letters: {cleaned_letters} (min length: {min_word_length})")
